...
- [Demo provider and model](tickets/008-demo-provider-and-model.md): BYOK OpenAI-compatible endpoint was for Clippy's own loop — **superseded**: Pi owns the provider now (omlx). `clippy/llmconfig.py` kept only as reference; not used by the brain.
...
- Graph-refactor leftovers: **P4** single roots-of-truth for paths/config/scratch (currently `CLIPPY_ROOT`/`SCRATCH_ROOT`/`make_scratch_dir` duplicated across subagent/brain/memory, config merged three ways, `llmconfig.py` dead) and **P5** a typed `Task` node for delegation (today a bare string + text protocol).
