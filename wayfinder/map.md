...
- Clippy's own chat loop is researched (ticket 002) but the loop code itself is not yet built; the sub-clippy delegation path (ticket 004) is the built proof. The demo's "one Clippy → one subagent spawn" acceptance is satisfied by `main.py --delegate`. Endpoint base_url/model for Clippy's own loop come from `clippy/llmconfig` / `~/.clippy/config.json`.
